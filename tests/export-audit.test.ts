...
describe('full source audit', () => {
  it('preserves removed custom values, source order and metadata separately', () => {
    ...
    expect(parsed.data[1]['Selected row']).toBe('no')
    expect(parsed.data[1].Decision).toBe('keep-one-row')
    expect(parsed.data[1]['Audit schema version']).toBe('3')
    expect(parsed.meta.fields).toContain('Matching score')
    expect(parsed.meta.fields).not.toContain('Confidence')
  ...
  it('lets keep-both override approval just like the reviewed export UI', () => {
    ...
    const parsed = Papa.parse<Record<string, string>>(csv, { header: true })
    expect(parsed.data.every(row => row.Decision === 'keep-all-rows')).toBe(true)
  })
})
